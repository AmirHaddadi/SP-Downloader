...
function initStreamingPlaceholder() {
  ...
  if (!searchBar || !urlInput) return;
  
  // Create streaming text overlay
  const streamOverlay = document.createElement('div');
  streamOverlay.className = 'search-stream-overlay';
  streamOverlay.style.cssText = `
    position: absolute;
    left: 6px;
    right: 50px;
    top: 0;
  ...
  streamText.style.cssText = `
    ...
    color: var(--text3);
    font-size: 0.95rem;
    animation: textStream 18s linear infinite;
    will-change: transform;
  `;
  
  // Hide streaming text when input is focused
...
const filterNavbar = $('filterNavbar');
...
function updateFilterBadges() {
  ...
  ['all','downloading','queued','paused','finalizing','done','error','cancelled'].forEach(s=>{
...
function applyFilters() {
  ...
  const history = Object.values(state.history).sort(sortByNewest);
  const source = state.scope === 'history' ? history : active;
  return source.filter(dl => {
    const statusMatch = state.filters.status === 'all' || dl.status === state.filters.status;
    const formatMatch = state.filters.format === 'all' || dl.format === state.filters.format;
    const platformMatch = state.filters.platform === 'all' || getPlatformKey(dl.url || '') === state.filters.platform;
    return statusMatch && formatMatch && platformMatch;
  });
}
...
function applyScope(scope) {
  ...
  state.filters = { status: 'all', format: 'all', platform: 'all' };
  document.querySelectorAll('.filter-nav-item').forEach(btn => {
...
function applyFilter(filterValue, filterType) {
  ...
  document.querySelectorAll(`.filter-nav-item[data-type="${filterType}"]`).forEach(btn => {
...
document.querySelectorAll('.filter-nav-item').forEach(btn => {
  btn.addEventListener('click', () => applyFilter(btn.dataset.filter, btn.dataset.type));
});
initStreamingPlaceholder();
applyScope('active');
loadDownloads();
...
function renderItem(dl) {
  ...
  const isActive   = ['downloading','finalizing','paused'].includes(dl.status);
...
<div class="dl-item ${isActive ? 'is-downloading' : ''}" data-id="${escAttr(key)}" data-status="${dl.status}" data-progress="${dl.progress}">
  ...
  <div class="dl-actions">
    ...
    ${dl.status === 'paused' ? `<button class="dl-btn dl-btn-primary" onclick="resumeDl(${jsArg(key)})" title="ادامه"><i class="fa-solid fa-play"></i><span>ادامه</span></button>` : ''}
    ${isActive ? `<button class="dl-btn" onclick="pauseDl(${jsArg(key)})" title="متوقف"><i class="fa-solid fa-pause"></i></button>` : ''}
...
window.pauseDl = async key => {
  const dl = getDownloadByKey(key);
  await api('/api/pause', { id: dl?.id, sessionId: dl?.sessionId || key });
  toast('<i class="fa-solid fa-pause"></i> دانلود متوقف شد');
};

window.resumeDl = async key => {
  const dl = getDownloadByKey(key);
  const btn = document.querySelector(`.dl-item[data-id="${CSS.escape(String(key))}"] .dl-btn-primary`);
  const originalHtml = btn ? btn.innerHTML : '';
  if (btn) { btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i><span>در حال ادامه...</span>'; btn.disabled = true; }
  try {
    const res = await api('/api/resume', { id: dl?.id, sessionId: dl?.sessionId || key });
    if (!res.ok) {
      toast(`<i class="fa-solid fa-circle-exclamation"></i> ${res.error || 'خطا در ادامه دانلود'}`);
      if (res.error?.includes('فایل') || res.error?.includes('پیدا نشد') || res.status === 'restarted') {
        const shouldRestart = confirm('فایل ناقص یافت نشد. آیا می‌خواهید از صفر شروع کنید؟');
        if (shouldRestart) {
          await api('/api/resume', { id: dl?.id, sessionId: dl?.sessionId || key, restart: true });
          toast('<i class="fa-solid fa-rotate-right"></i> دانلود از صفر شروع شد');
        }
      }
    } else {
      toast(res.status === 'restarted' ? '<i class="fa-solid fa-rotate-right"></i> دانلود از صفر شروع شد' : '<i class="fa-solid fa-play"></i> دانلود ادامه یافت');
    }
  } catch (e) {
    toast(`<i class="fa-solid fa-circle-exclamation"></i> خطا: ${escHtml(e.message)}`);
  } finally {
